...
def interpret_output_yolov1(output, img_width, img_height):
    ...
    scales = np.reshape(
        output[grid_size * grid_size * num_class:grid_size * grid_size * num_class + grid_size * grid_size * 2],
        (grid_size, grid_size, 2))
    boxes = np.reshape(output[grid_size * grid_size * num_class + grid_size * grid_size * 2:],
                       (grid_size, grid_size, 2, 4))
    offset = np.transpose(np.reshape(np.array([np.arange(grid_size)] * grid_size * 2), (2, grid_size, grid_size)),
                          (1, 2, 0))
...
def show_results(img, results, img_width, img_height, model_age, model_gender, model_emotion):
    ...
    for i in range(len(results)):
        ...
        cv2.putText(img_cp, results[i][0] + ' : %.2f' % results[i][5], (xmin + 5, ymin - 7), cv2.FONT_HERSHEY_SIMPLEX,
                    0.5, (0, 0, 0), 1)
        ...
        if (model_age != None):
            ...
            cv2.putText(target_image, "Age : " + label, (xmin2, ymax2 + offset), cv2.FONT_HERSHEY_COMPLEX_SMALL, 0.8,
                        (0, 0, 250));
            offset = offset + 16
            age = int(age)
        ...
        if (model_gender != None):
            ...
            cv2.putText(target_image, "Gender : %.2f" % prob_gender_keras + " " + lines_gender[cls_gender_keras],
                        (xmin2, ymax2 + offset), cv2.FONT_HERSHEY_COMPLEX_SMALL, 0.8, (0, 0, 250));
            offset = offset + 16
            print(pred_gender_keras[0], pred_gender_keras[1])
            if (pred_gender_keras[0] > pred_gender_keras[1]):
                print('female')
                res_gender = 'женщина'
            else:
                print('male')
...
def main_module(name):
    ...
    res_age, res_gender = show_results(img_cv, results, img.shape[1], img.shape[0], model_age, model_gender,
                                       model_emotion)
